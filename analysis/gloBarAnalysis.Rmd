...
atl_comm_path <- file.path(p_dir, "parrotfish_BMUs_Atlantic_reefBudgetGrazingAdded.csv")
...
to_loc3_bar <- function(region, loc2, subRegion, siteName, basin) {
  ...
  dplyr::case_when(
    ...
    bsn == "atlantic" & loc2 == "keys" ~ paste(
      loc2,
      ifelse(first3_letters_lower(site_norm) == "che", "uki",
             first3_letters_lower(site_norm)),
      sep = "-"
    ),
...
to_loc3_parrot <- function(region, loc2, subRegion, siteName, basin) {
  ...
  dplyr::case_when(
    ...
    bsn == "atlantic" & loc2 == "keys" & subRegion == "lower"  & site_norm == "inshore"  ~ "keys-lki",
    bsn == "atlantic" & loc2 == "keys" & subRegion == "lower"  & site_norm == "offshore" ~ "keys-lko",
    bsn == "atlantic" & loc2 == "keys" & subRegion == "middle" & site_norm == "inshore"  ~ "keys-mki",
    bsn == "atlantic" & loc2 == "keys" & subRegion == "middle" & site_norm == "offshore" ~ "keys-mko",
    bsn == "atlantic" & loc2 == "keys" & subRegion == "upper"  & site_norm == "inshore"  ~ "keys-uki",
    bsn == "atlantic" & loc2 == "keys" & subRegion == "upper"  & site_norm == "offshore" ~ "keys-uko",
...
parrotCommunityData_raw <- read_bind_csv(c(atl_comm_path, pac_comm_path), ct) %>%
  drop_all_na_rows()

# Read & bind parrot FXN data
parrotFXNData_raw <- read_bind_csv(c(atl_fxn_path, pac_fxn_path), ct) %>%
  drop_all_na_rows()
...
#Parrot Data QAQC
parrotCommunityData <- parrotCommunityData_raw
parrotFXNData <- parrotFXNData_raw
...
# Create parrot functional group loc fields
...
```{r mass_vs_volume_fill_and_plot, fig.width=10, fig.height=3.8, echo=FALSE, message=FALSE, warning=FALSE}
# =========================
# 0) Inputs
# =========================
...
parrotFXNData_clean <- parrotFXNData %>%
  mutate(
    ...
    `loc1.2` = if_else(is.na(loc2) | loc2 == "", loc1, paste0(loc1, ".", loc2)) |>
      stringr::str_squish(),
    fxnGroup = case_when(
      stringr::str_to_lower(fxnGroup) %in% c("scraper", "scrapers")     ~ "scraper",
      stringr::str_to_lower(fxnGroup) %in% c("excavator", "excavators") ~ "excavator",
      TRUE ~ stringr::str_to_lower(fxnGroup)
    )
  )
...
comm_summary <- parrotCommunityData_clean %>%
  ...
  summarise(
    ...
    # Bioerosion (kg m-2 yr-1)
    comm_bioerosion_mean = mean(bioerosion_kg_m2_yr1, na.rm = TRUE),
    comm_bioerosion_sd   = sd(bioerosion_kg_m2_yr1,   na.rm = TRUE),
    comm_bioerosion_n    = sum(!is.na(bioerosion_kg_m2_yr1)),
...
fxn_summary <- parrotFXNData_clean %>%
  ...
  tidyr::pivot_wider(
    names_from  = fxnGroup,
    values_from = c(
      fxn_density_mean, fxn_density_sd, fxn_density_n,
...
# ---------- Neat kable (Community/Excavators/Scrapers ABOVE Density/Biomass/Bioerosion) ----------
display_tbl <- parrot_loc3_summary %>%
  mutate(
    # Density & biomass → 1 decimal
    across(matches("^(comm|fxn)_(density|biomass)_(mean|sd)$"), ~ round(., 1)),
    # Bioerosion → 2 decimals
    across(matches("^comm_bioerosion_(mean|sd)$"), ~ round(., 2)),
    # N as integers
    across(ends_with("_n"), ~ as.integer(.))
  )

# 5 ID cols + 21 numeric cols
align_vec <- c(rep("l", 5), rep("r", 21))
...
kableExtra::kbl(
  ...
  kableExtra::add_header_above(
    c(" " = 5,
      "Density" = 3, "Biomass" = 3, "Bioerosion" = 3,
      "Density" = 3, "Biomass" = 3,
      "Density" = 3, "Biomass" = 3),
    bold = FALSE
  ) %>%
  kableExtra::add_header_above(
    c(" " = 5, "Community" = 9, "Excavators" = 6, "Scrapers" = 6)
  ) %>%
...
# 4 ID cols + (Community 9) + (Excavators 6) + (Scrapers 6) = 25 total cols
align_vec <- c(rep("l", 4), rep("r", 21))
...
kableExtra::kbl(
  ...
  # Inner row (Density/Biomass/Bioerosion)
  kableExtra::add_header_above(
    c(" " = 4,
      "Density" = 3, "Biomass" = 3, "Bioerosion" = 3,
      "Density" = 3, "Biomass" = 3,
  ...
  # Outer row (Community / Excavators / Scrapers)
  kableExtra::add_header_above(
    c(" " = 4, "Community" = 9, "Excavators" = 6, "Scrapers" = 6)
  ) %>%
  kableExtra::kable_styling(
    full_width = FALSE,
    position = "left",
...
for (resp in responses) {
  ...
  # Bioerosion: only exists for Atlantic in your data.
  # Filter to rows where bioerosion is present (prevents Pacific all-NA from causing issues).
  comm_bioerosion <- safe_plot_loc2(
    dplyr::filter(barParrotStatsLoc2, !is.na(comm_bioerosion_mean)),
    x = "comm_bioerosion_mean", y = resp,
    sd_x = "comm_bioerosion_sd", sd_y = paste0("se", sub("mean", "", resp)),
  ...
  # Build grid:
  # - Community now is 3 columns (biomass, density, bioerosion)
  # - Excavator & Scraper remain 2 columns (biomass, density)
  grid <- cowplot::plot_grid(
    ...
    cowplot::plot_grid(
      row_title("Excavator"),
      cowplot::plot_grid(excavator_biomass, excavator_density, ncol = 2),
      ncol = 1, rel_heights = c(0.1, 1)
    ),
    cowplot::plot_grid(
      row_title("Scraper"),
      cowplot::plot_grid(scraper_biomass, scraper_density, ncol = 2),
      ncol = 1, rel_heights = c(0.1, 1)
    ),
    ncol = 1
  )
  ...
  grid_title <- cowplot::ggdraw() +
    cowplot::draw_label(
      paste("Δ Response:", gsub("meanDelta", "Δ ", resp)),
...
for (resp in responses) {
  ...
  # Y-axis label (Δ response) (not used below, leaving intact)
  ylab_expr <- bquote(Delta~.(gsub("meanDelta", "", resp))~(units))

  # Community — biomass, density, bioerosion
  ...
  # Bioerosion exists only for Atlantic (and may be all NA for Pacific) — filter to present rows
  comm_bioerosion <- safe_plot_loc3(
    dplyr::filter(barParrotStatsLoc3, !is.na(comm_bioerosion_mean)),
    x = "comm_bioerosion_mean", y = resp,
    sd_x = "comm_bioerosion_sd", sd_y = paste0("se", sub("mean", "", resp)),
    xlab = expression(Parrot~bioerosion~(kg~m^-2~yr^-1)),
    ylab = NULL, group = "basin",
  ...
  # Build grid:
  # - Community now 3 columns (biomass, density, bioerosion)
  # - Excavator & Scraper remain 2 columns
  grid <- cowplot::plot_grid(
    cowplot::plot_grid(
      row_title("Community"),
      cowplot::plot_grid(comm_biomass, comm_density, comm_bioerosion, ncol = 3),
...
arag_long <- siteStatsBottleEnv %>%
  dplyr::select(
    ...
    mean_AragSatState, se_AragSatState,
    seAccretion, seGrazing, seMacroboring, seDeltaMass, seDeltaVolume, seDeltaDensity
  ) %>%
  tidyr::pivot_longer(
  ...
  dplyr::mutate(
    ...
    response_se = dplyr::case_when(
      ...
      response_col == "meanDeltaMass"   ~ seDeltaMass,
...
# 2) Ensure clean 'basin' and 'loc1.2'
...
# 3) Join raw gravity to barData
barData <- barData %>%
  dplyr::left_join(gravityData, by = "loc3")

# 4) Response variables + labels
responses <- c(
  accretion_mm3cm2y   = "accretion",
  grazing_mm3cm2y     = "grazing",
  macroboring_mm3cm2y = "macroboring",
...
# 5) Long-format data for gravity analysis
gravity_long <- barData %>%
  ...
  dplyr::mutate(
    metric         = dplyr::recode(response_col, !!!responses),
    response_value = as.numeric(response_value),
    totalGravity   = as.numeric(totalGravity),
    basin          = tolower(as.character(basin)),
