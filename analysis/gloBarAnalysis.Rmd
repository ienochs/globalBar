...
bottleData_loc2 <- read_csv(bottleLoc2_path, show_col_types = FALSE)
...
# ====== 2) Make sure everything numeric is "clean" ======
clean_numeric <- function(df) {
  df %>%
    dplyr::mutate(
      dplyr::across(
...
# ======  3) Build joined table with all response + bottle env data ====== 
...
resp_cols   <- unname(responses)   # underlying column names
resp_labels <- names(responses)    # "accretion", "grazing", ...
...
arag_long <- siteStatsBottleEnv %>%
  ...
  dplyr::mutate(
    # response_col is the column name (e.g. "meanAccretion")
    metric = dplyr::recode(response_col, !!!setNames(resp_labels, resp_cols)),
    metric = factor(metric, levels = resp_labels),   # lock facet order
...
# ====== 5) Helper functions for per-basin regression plots (same pattern as human-pressure code) ======
interpret_model <- function(R2, p) {
  if (is.na(R2) || is.na(p)) return("insufficient data")
  if (p >= 0.05) return("not significant")
  if (R2 < 0.10) return("weak but significant")
...
make_basin_fit_data <- function(df, xvar) {
  df_base <- df %>% dplyr::filter(!is.na(response_value))
  
  dplyr::bind_rows(
    df_base %>% dplyr::mutate(fit_group = "All data"),
    df_base %>%
      dplyr::filter(basin == "atlantic") %>%
      dplyr::mutate(fit_group = "Atlantic"),
    df_base %>%
      dplyr::filter(basin == "pacific") %>%
      dplyr::mutate(fit_group = "Pacific")
  ) %>%
    dplyr::filter(!is.na(.data[[xvar]])) %>%
    dplyr::mutate(
      fit_group = factor(fit_group,
                         levels = c("All data", "Atlantic", "Pacific"))
    )
}
...
facet_lm_stats_basin <- function(df, xvar) {
  ...
  df_long %>%
    ...
    dplyr::reframe({
      ...
      x_pos <- x_min + (if (xr == 0) 0.02 * (if (x_max == 0) 1 else abs(x_max))
                        else 0.02 * xr)
      y_pos <- y_max - (if (yr == 0) 0.02 * (if (y_max == 0) 1 else abs(y_max))
                        else 0.02 * yr)
      ...
      label <- paste0(
        ...
        ", p = ", ifelse(is.na(p), "NA", formatC(p, format = "e", digits = 2)),
        ", n = ", n,
        "\nslope = ", ifelse(is.na(slope), "NA",
                             formatC(slope, format = "g", digits = 3)),
        "  (", interp, ")"
      )
...
# ====== 6) Plot responses vs Ωarag (All / Atlantic / Pacific fits) ======
plot_arag <- arag_long %>%
  plot_response_vs_x_basin(
    xvar       = "mean_AragSatState",
    x_label    = expression(Omega[arag]),
    title_text = expression(Omega[arag] ~ "vs. Bar Responses (All / Atlantic / Pacific fits)")
  )
...
# This inner_join automatically:
# - drops loc3 with no bottle data
# - keeps loc3 with a single bottle (n_bottle = 1) — they'll still appear in plots
...
resp_cols   <- unname(responses)    # actual column names
resp_labels <- names(responses)     # "accretion", "grazing", ...
...
arag_loc3_long <- siteStatsBottleEnvLoc3 %>%
  ...
  dplyr::mutate(
    metric = dplyr::recode(response_col, !!!setNames(resp_labels, resp_cols)),
    metric = factor(metric, levels = resp_labels),  # facet order: accretion → grazing → macroboring → mass → volume
    response_value    = suppressWarnings(as.numeric(response_value)),
    mean_AragSatState = suppressWarnings(as.numeric(mean_AragSatState)),
    basin             = tolower(as.character(basin))
  )

# ====== 5) Plot responses vs Ωarag (All / Atlantic / Pacific fits) ======
plot_arag_loc3 <- arag_loc3_long %>%
  plot_response_vs_x_basin(
    xvar       = "mean_AragSatState",
    x_label    = expression(Omega[arag]),
    title_text = expression(Omega[arag] ~ "vs. Bar Responses by loc3 (All / Atlantic / Pacific fits)")
  )

print(plot_arag_loc3)
```

```{r accretion_vs_arag_loc2_lm_loccolors, fig.width=7, fig.height=7, echo=FALSE}

library(dplyr)
library(ggplot2)

# 1) Subset to rows with needed data and only Atlantic/Pacific
acc_arag_loc2 <- siteStatsBottleEnv %>%
  dplyr::filter(
    !is.na(meanAccretion),
...
# 2) Fit separate LMs for each basin and extract stats
lm_stats <- acc_arag_loc2 %>%
  dplyr::group_by(basin) %>%
  dplyr::summarise(
    n = sum(complete.cases(mean_AragSatState, meanAccretion)),
    lm_ok = n > 2 &&
      stats::var(mean_AragSatState, na.rm = TRUE) > 0 &&
      stats::var(meanAccretion, na.rm = TRUE) > 0,
    model = if (lm_ok[1]) list(stats::lm(meanAccretion ~ mean_AragSatState))
            else list(NULL),
    .groups = "drop"
  ) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(
    R2 = if (!is.null(model)) summary(model)$r.squared else NA_real_,
    p  = if (!is.null(model)) summary(model)$coefficients[2, 4] else NA_real_,
    a  = if (!is.null(model)) stats::coef(model)[1] else NA_real_,  # intercept
    b  = if (!is.null(model)) stats::coef(model)[2] else NA_real_,  # slope
    label = paste0(
      "y = ", formatC(a, digits = 3, format = "f"),
      " + ", formatC(b, digits = 3, format = "f"), "x\n",
...
# 3) Choose label positions within each basin's data range
label_pos <- acc_arag_loc2 %>%
  dplyr::group_by(basin) %>%
  dplyr::summarise(
    x_min = min(mean_AragSatState, na.rm = TRUE),
    x_max = max(mean_AragSatState, na.rm = TRUE),
    y_min = min(meanAccretion, na.rm = TRUE),
    y_max = max(meanAccretion, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  dplyr::mutate(
    xr = x_max - x_min,
  ...
  dplyr::select(basin, x, y)

lm_labels <- dplyr::left_join(lm_stats, label_pos, by = "basin")

# basins where LM is significant
sig_basins <- lm_stats %>%
  dplyr::filter(is_sig, lm_ok) %>%
  dplyr::pull(basin)

# 4) Color scale from loc_colors (tied to loc1.2)
color_scale <- if (exists("loc_colors", inherits = TRUE)) {
  scale_color_manual(values = loc_colors, na.translate = FALSE)
} else {
  scale_color_discrete()
}

# 5) Final plot: points, x/y error bars, significant basin-specific LMs, labels
gg_acc_arag_loc2_lm <- ggplot(
  acc_arag_loc2,
  aes(
    x = mean_AragSatState,
    y = meanAccretion,
  ...
  # label points by loc2 (Title Case)
  geom_text(
    aes(label = tools::toTitleCase(as.character(loc2))),
    vjust = -0.8,
    show.legend = FALSE,
    size = 5
  ) +

  # vertical error bars (accretion)
  geom_errorbar(
    aes(
      ymin = meanAccretion - seAccretion,
      ymax = meanAccretion + seAccretion
    ),
    width = 0
  ) +

  # horizontal error bars (Ωarag)
  geom_errorbarh(
    aes(
      xmin = mean_AragSatState - se_AragSatState,
      xmax = mean_AragSatState + se_AragSatState
    ),
    height = 0
  ) +

  # basin-specific regression lines (only for significant basins)
  geom_smooth(
    data = acc_arag_loc2 %>% dplyr::filter(basin %in% sig_basins),
    aes(
      x = mean_AragSatState,
      y = meanAccretion,
      linetype = basin,
  ...
  # LM labels (equation, R², p)
  geom_text(
    data = lm_labels %>% dplyr::filter(is_sig, lm_ok),
  ...
  # NEW: force ATL = square, PAC = circle
  scale_shape_manual(values = c("atlantic" = 15,   # square
                                "pacific"  = 16)) + # circle

  scale_linetype_discrete(name = "Basin") +
...
print(gg_acc_arag_loc2_lm)

ggsave(
  filename = "accretion_vs_arag_loc2_lm.pdf",
  plot = gg_acc_arag_loc2_lm,
  device = cairo_pdf,   # high-quality text rendering
  width = 7,
  height = 7,
  units = "in"
)
