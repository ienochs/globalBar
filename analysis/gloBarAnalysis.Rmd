...
deltaMassGrazingStats <- allBarData_clean %>%
  ...
  group_by(siteName) %>%
  summarise(meanGrazing = mean(grazing_mm3cm2y), sdGrazing = sd(grazing_mm3cm2y), meanDeltaMass = mean(deltaMass_mgcm2y), sdDeltaMass = sd(deltaMass_mgcm2y), region = region, n = n())
deltaMassGrazingStats
deltaMassGrazingStatsPlot <- ggplot(deltaMassGrazingStats, aes(x = meanGrazing, y = meanDeltaMass)) +
  geom_point(aes(color = factor(region))) +  # Scatterplot
  geom_errorbar(aes(ymin=meanDeltaMass-sdDeltaMass, ymax=meanDeltaMass+sdDeltaMass), width=.1) +
  geom_errorbar(aes(xmin=meanGrazing-sdGrazing, xmax=meanGrazing+sdGrazing), width=.1) +
