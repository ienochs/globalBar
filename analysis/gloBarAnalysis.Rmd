...
#Fix depths in the Pacific
depth_csv_path <- file.path(data_dir, "barData_pac_depths.csv")
# --- read + prep depth table ---
pac_depths <- readr::read_csv(depth_csv_path, show_col_types = FALSE) %>%
  dplyr::mutate(siteName = trimws(siteName))

# If your CSV depth column is not literally named "depth", set it here:
# (leave as-is if your CSV column name is already depth)
depth_col_in_csv <- "depth"

stopifnot("siteName" %in% names(pac_depths))
stopifnot(depth_col_in_csv %in% names(pac_depths))

pac_depths2 <- pac_depths %>%
  dplyr::select(siteName, depth_new = all_of(depth_col_in_csv)) %>%
  dplyr::filter(!is.na(siteName), siteName != "") %>%
  # if duplicates exist, keep the first non-NA depth
  dplyr::group_by(siteName) %>%
  dplyr::summarise(depth_new = dplyr::first(na.omit(depth_new)), .groups = "drop")

# --- normalize siteName in barData_raw ---
barData_raw <- barData_raw %>%
  dplyr::mutate(siteName = trimws(siteName))

stopifnot("siteName" %in% names(barData_raw))
stopifnot("basin" %in% names(barData_raw))
stopifnot("depth" %in% names(barData_raw))  # change if your depth field has a different name

# --- unmatched lists (both directions) ---
pac_sites_in_bar <- barData_raw %>%
  dplyr::filter(tolower(basin) == "pacific") %>%
  dplyr::distinct(siteName) %>%
  dplyr::filter(!is.na(siteName), siteName != "")

sites_in_csv <- pac_depths2 %>%
  dplyr::distinct(siteName) %>%
  dplyr::filter(!is.na(siteName), siteName != "")

siteNames_missing_in_csv <- setdiff(pac_sites_in_bar$siteName, sites_in_csv$siteName)
siteNames_missing_in_bar <- setdiff(sites_in_csv$siteName, pac_sites_in_bar$siteName)

unmatched_siteNames <- list(
  in_barData_raw_pacific_but_not_in_csv = sort(siteNames_missing_in_csv),
  in_csv_but_not_in_barData_raw_pacific = sort(siteNames_missing_in_bar)
)

# --- update depth ONLY for pacific rows ---
barData_raw_updated <- barData_raw %>%
  dplyr::left_join(pac_depths2, by = "siteName") %>%
  dplyr::mutate(
    depth = dplyr::if_else(tolower(basin) == "pacific" & !is.na(depth_new), depth_new, depth)
  ) %>%
  dplyr::select(-depth_new)

# --- results ---
# 1) updated data frame:
barData_raw <- barData_raw_updated

# 2) list of siteNames with no match either direction:
unmatched_siteNames
```
