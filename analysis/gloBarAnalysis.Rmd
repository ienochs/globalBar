...
# 1) Fit species-specific LMs (measured only) for ofav & plob
lm_models <- barData %>%
  filter(
  ...
  group_by(species) %>%
  summarise(
    model = list(lm(as.formula(paste0(yvar, " ~ ", xvar)), data = cur_data())),
    .groups = "drop"
  )
...
to_fill <- barData %>%
  ...
  left_join(lm_models, by = "species") %>%
  rowwise() %>%
  mutate(
    volumeBlockPre_pred = predict(model, newdata = cur_data())
  ) %>%
  ungroup()

# tagNum lists (calculated)
tagNums_calculated <- to_fill %>%
  select(species, tagNum) %>%
  arrange(species, tagNum)

tagNums_ofav <- tagNums_calculated %>% filter(species == "ofav") %>% pull(tagNum)
tagNums_plob <- tagNums_calculated %>% filter(species == "plob") %>% pull(tagNum)

# 3) Merge predictions back into barData (fill ONLY missing volumeBlockPre)
#    Also add a provenance flag
barData <- barData %>%
  left_join(
    to_fill %>% select(tagNum, volumeBlockPre_pred),
    by = "tagNum"
  ) %>%
  mutate(
    volumeBlockPre_source = case_when(
  ...
  select(-volumeBlockPre_pred)
# 4) Build 2-panel plot for species that have BOTH mass & volume (after fill)
#    LM stats (equation/R2/p) are computed from MEASURED-only complete cases
# =========================
plot_df <- barData %>%
  filter(!is.na(.data[[xvar]]), !is.na(.data[[yvar]]), !is.na(species))

# pick the 2 species that have BOTH fields available (after fill)
species2 <- plot_df %>%
  count(species, name = "n_complete") %>%
  arrange(desc(n_complete), species) %>%
  slice_head(n = 2) %>%
  pull(species)

plot_df2 <- plot_df %>% filter(species %in% species2)

# lm stats per species from MEASURED complete cases only (so imputation doesn't bias fit)
lm_stats <- barData %>%
  filter(
    species %in% species2,
    !is.na(.data[[xvar]]),
  ...
  group_by(species) %>%
  do({
    m  <- lm(as.formula(paste0(yvar, " ~ ", xvar)), data = .)
    sm <- summary(m)
    tibble(
      intercept = coef(m)[1],
      slope     = coef(m)[2],
...
# =========================
ggplot(plot_df2, aes(x = .data[[xvar]], y = .data[[yvar]])) +
  geom_point(aes(shape = volumeBlockPre_source), size = 2, alpha = 0.85) +
...
# =========================
# 6) Print tagNum lists of calculated values (as plain output)
# =========================
