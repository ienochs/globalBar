...
```{r boxplots_by_loc2_panels, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
library(dplyr)
library(tidyr)
library(ggplot2)
library(patchwork)
...
# ===== Loop over islands =====
for (this_loc2 in sort(unique(barData$loc2))) {

  df_loc2 <- barData %>%
    filter(loc2 == this_loc2)

  if (nrow(df_loc2) == 0) next

  # ---------- PANEL 1: Accretion / Macroboring / Grazing ----------
  df_panel1 <- df_loc2 %>%
    select(all_of(panel1_vars)) %>%
    pivot_longer(
      cols = everything(),
      names_to = "metric",
      values_to = "value"
  ...
  p1 <- ggplot(df_panel1, aes(x = metric, y = value)) +
    geom_boxplot(
      fill = "grey30",
      color = "grey10",
      outlier.alpha = 0.4,
    ...
    theme(
      ...
      axis.text.x       = element_text(size = 27, angle = 45, hjust = 1),
  ...
  # ---------- PANEL 2: Volume ----------
  p2 <- ggplot(df_loc2, aes(x = "Volume", y = .data[[volume_var]])) +
    geom_boxplot(
      fill = "grey90",
      color = "grey10",
      outlier.alpha = 0.4,
  ...
  # ---------- Combine for this island (3:1 widths) ----------
  p_combined <- (p1 | p2) + plot_layout(widths = c(3, 1))
  ...
  # ---------- Save to PDF with 1.5:1 aspect ratio ----------
  out_file <- file.path(out_dir, paste0("regionPlot_", this_loc2, ".pdf"))
  ggsave(
    filename = out_file,
    plot     = p_combined,
    width    = 6,   # any values with width/height = 1.5 are fine
    height   = 6,
    units    = "in",
    device   = cairo_pdf
  )
...
# ===== GLOBAL SHARED X LIMITS (for horizontal plots) =====
shared_xlim <- range(
  c(
    barData$accretion_mm3cm2y,
    barData$macroboring_mm3cm2y,
...
# ===== Harmonious, muted colors for metrics =====
metric_colors <- c(
  "Accretion"    = "#2E8B57",  # sea-green
  "Macroboring"  = "#E69F00",  # muted amber
  "Grazing"      = "#C44E52",  # muted brick red
  "Total volume" = "#222222"   # soft black
)

# Vertical offsets within each loc2 row
# Grazing (lowest) < Macroboring < Accretion < Total volume (highest)
offset_map <- c(
  "Grazing"      = -0.15,
  "Macroboring"  = -0.05,
  "Accretion"    =  0.05,
...
for (this_loc1 in sort(unique(barData$loc1))) {
  ...
  df_loc1 <- barData %>%
    filter(loc1 == this_loc1)

  if (nrow(df_loc1) == 0) next

  # ---------- Long format, then summarise to loc2-level mean + SD ----------
  df_long <- df_loc1 %>%
    select(loc2, all_of(c(panel1_vars, volume_var))) %>%
    pivot_longer(
      cols      = -loc2,
      names_to  = "metric",
      values_to = "value"
    ) %>%
    group_by(loc2, metric) %>%
    summarise(
  ...
  df_long <- df_long %>%
    mutate(
      ...
      sd          = ifelse(is.na(sd), 0, sd)   # single obs -> zero-width bar
    )

  # ---------- Pretty loc2 labels (capitalized) ----------
  ...
  p_loc1 <- ggplot(
    df_long,
    aes(x = mean, y = y_pos, color = metric_label)
  ) +
    geom_vline(
      xintercept = 0,
      linetype   = "dashed",
    ...
    # Error bars WITHOUT caps (all except Total volume)
    geom_errorbarh(
      data = df_long %>% dplyr::filter(metric_label != "Total volume"),
      aes(
        xmin = mean - sd,
        xmax = mean + sd,
        y    = y_pos
    ...
    # Error bars WITH caps (Total volume ONLY)
    geom_errorbarh(
      data = df_long %>% dplyr::filter(metric_label == "Total volume"),
      aes(
        xmin = mean - sd,
      ...
      height    = 0.08,  # small caps only for Total volume
      linewidth = 1.6
    ) +
    geom_point(
      aes(
        size = ifelse(metric_label == "Total volume", 5.5, 3.8)
      ),
      alpha = 0.9
    ) +
    scale_color_manual(
    ...
    scale_y_continuous(
      ...
      labels = loc2_labels_pretty   # <- capitalized loc2 names
    ) +
    xlab(expression(Delta~volume~(mm^3~cm^{-2}~y^{-1}))) +
    ylab(NULL) +
    ggtitle(loc1_pretty) +
  ...
  # ---------- Save to PDF ----------
  out_file <- file.path(out_dir, paste0("regionPlot_loc1_", this_loc1, ".pdf"))
  ggsave(
    filename = out_file,
    plot     = p_loc1,
    width    = base_width,
...
suppressPackageStartupMessages({
  ...
  library(tibble)
  library(stringr)
  library(patchwork)  # for arranging metric panels
})
...
### stats analysis
```{r additionalstats, fig.width=12, fig.height=18, out.width="100%", echo=FALSE}
## =========================
## 0) Packages
## =========================
library(dplyr)
